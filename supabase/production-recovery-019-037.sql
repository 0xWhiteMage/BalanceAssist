-- SQL Editor fallback only; not for cleanup.
-- Do not directly manage Supabase Storage relations or buckets.
...
DECLARE
  v_recorded text[];
BEGIN
  SELECT array_agg(version ORDER BY version) INTO v_recorded
  FROM public.schema_migrations
  WHERE version IN ('019', '020', '021', '022', '023', '024', '025', '026', '027', '028', '029', '030', '031', '032', '033', '034', '035', '036', '037');

  IF v_recorded IS NOT NULL THEN
    RAISE EXCEPTION 'recovery migrations 019-037 are already recorded: %', v_recorded;
  END IF;
END
$$;
...
BEGIN
  ...
  ('036', '036_atomic_mutations.sql'),
  ('037', '037_scheduler_health.sql')
  ) AS expected(version, filename)
    ON actual.version = expected.version
   AND actual.filename = expected.filename;

  IF v_actual_count <> 19 OR v_matching_count <> 19 THEN
    RAISE EXCEPTION 'schema_migrations verification failed for 019-037';
  END IF;
END
$$;
