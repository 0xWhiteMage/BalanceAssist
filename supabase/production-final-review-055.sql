BEGIN;
SELECT pg_advisory_xact_lock(90442055);
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = '054' AND filename = '054_human_contact_consent.sql') THEN
    RAISE EXCEPTION 'final review migration baseline 054 is not recorded with its reviewed filename';
  END IF;
  IF to_regclass('public.sessions') IS NULL OR to_regclass('public.crm_leads') IS NULL OR to_regclass('public.crm_lead_revisions') IS NULL OR to_regprocedure('public.finalize_session_lead(uuid)') IS NULL THEN
    RAISE EXCEPTION 'final review migration baseline schema signatures are missing';
  END IF;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = '055') THEN
    RAISE EXCEPTION 'reviewed final review migration 055 is already recorded';
  END IF;
END $$;

-- BEGIN 055 055_final_review_approval.sql
...
BEGIN
  ...
  IF v_session.deletion_state <> 'active' OR v_consent.notice_version <> '1.1' THEN
    RETURN QUERY SELECT true, false, v_status, v_score, v_next, v_lead_id, v_handoff_id, null::uuid, null::integer, null::integer, false, null::text, null::text;
    RETURN;
  END IF;
...
END $$;
-- END 055 055_final_review_approval.sql
INSERT INTO public.schema_migrations (version, filename) VALUES ('055', '055_final_review_approval.sql');

DO $$
DECLARE function_result text;
BEGIN
  SELECT pg_get_function_result('public.finalize_session_lead(uuid)'::regprocedure) INTO function_result;
  IF function_result IS NULL OR position('approval_input_hash text' IN function_result) = 0
    OR position('approved_reference_set_hash text' IN function_result) = 0
    OR NOT EXISTS (SELECT 1 FROM public.schema_migrations WHERE version = '055' AND filename = '055_final_review_approval.sql') THEN
    RAISE EXCEPTION 'final review migration verification failed';
  END IF;
END $$;
COMMIT;
