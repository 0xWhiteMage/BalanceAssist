...
```bash
Apply the full incremental migration chain in order, through `042_deletion_recovery_ownership.sql` (except intentionally absent `005`). Do not combine it with legacy snapshot `000_full_schema.sql`.
```
