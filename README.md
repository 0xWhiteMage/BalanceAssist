...
3. Keep the Vercel project connected to Git for automatic branch previews. `vercel.json` must set `git.deploymentEnabled.main` to `false` so a `main` push cannot deploy routes before migration `061` has installed their required RPCs; protected production releases continue through Vercel CLI.
...
Set the protected `production` environment variable `VERCEL_GIT_DEPLOYMENTS_DISABLED_AT` to a UTC ISO-8601 audit timestamp (for example, `2026-07-14T12:00:00Z`). The release gate rejects absent, malformed, future, or more-than-90-day-old attestations and confirms the expected Vercel project is linked to this repository while `vercel.json` disables automatic deployment of `main`. Branch previews remain automatic; protected production releases use Vercel CLI. Recheck this before each release and quarterly.
