...
The authoritative schema is the incremental chain from `001_initial_schema.sql` through `025_in_flight_handoff_retention.sql` (including the intentionally absent `005` version). `000_full_schema.sql` is a legacy snapshot and must not be combined with the incremental chain. Temporary-draft expiry is invoked every five minutes by the GitHub Actions worker. An active dispatcher claim is allowed to finish its in-flight external transfer; expiry or revoked consent suppresses only unclaimed handoffs and cannot retract an already claimed transfer.
