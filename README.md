...
Rate-limit retention is not part of the request path. When `pg_cron` is installed, migration `020` schedules a daily bounded prune. Otherwise schedule `select public.prune_api_rate_limits(500)` daily with the deployment's database maintenance facility. Private attachment object cleanup is also invoked by the authenticated session-expiry worker; it only reports deletion after Storage removes the object, and reports failed removals for retry.
