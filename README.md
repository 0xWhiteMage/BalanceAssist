...
Rate-limit retention is not part of the request path. When `pg_cron` is installed, migration `020` schedules a daily bounded prune. Otherwise schedule `select public.prune_api_rate_limits(500)` daily with the deployment's database maintenance facility. Migration `030` provisions the private `temporary-attachments` bucket, removes matching permissive API policies, and revokes direct `anon` and `authenticated` access to Storage objects. It records Storage readiness and uploads fail closed unless readiness is `ready`. The authenticated session-expiry worker removes stored objects before expiring metadata; if metadata persistence and rollback deletion both fail, it retains an opaque cleanup record (object key, bucket, checksum, and expiry only) for later retry.
