...
The authoritative schema is the incremental chain from `001_initial_schema.sql` through `027_handoff_send_reservations.sql` (including the intentionally absent `005` version). `000_full_schema.sql` is a legacy snapshot and must not be combined with the incremental chain. Temporary-draft expiry is invoked every five minutes by the GitHub Actions worker. A dispatcher reserves `sending` for 90 seconds before its 45-second Telegram call; expiry or revoked consent suppresses only unclaimed handoffs and cannot retract an already accepted external transfer.
