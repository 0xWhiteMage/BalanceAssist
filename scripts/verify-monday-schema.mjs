...
const query = 'query BoardSchema($boardIds: [ID!], $boardId: ID!) { me { account { id } } boards(ids: $boardIds, capabilities: []) { id board_kind workspace { id } columns { id type settings_str } capabilities } validations(id: $boardId, type: board) { required_column_ids rules } }';
...
if (body.data.me?.account?.id !== schema.accountId || board?.id !== schema.boardId || board?.board_kind !== schema.boardKind || board?.workspace?.id !== schema.workspaceId || board?.capabilities?.item_name !== true) throw new Error('Monday board identity or item-name capability does not match the checked-in contract');
