...
const { client, hasConfig } = vi.hoisted(() => ({
  ...
  client: { rpc: vi.fn(async (name: string) => name === 'prune_processed_telegram_updates'
    ? { data: 3, error: null }
    : { data: { deleted_sessions: 2, deferred_sessions: 1, released_claims: 0 }, error: null }) }
}));
...
describe('POST /api/internal/session-expiry', () => {
  ...
  test('returns safe purge and active-claim deferral counts after authenticated cleanup', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ ok: true, deletedSessions: 2, deferredSessions: 1, releasedClaims: 0, prunedTelegramUpdates: 3 });
  ...
  test('reports private object cleanup only after the storage adapter deletes objects', async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ ok: true, deletedSessions: 2, deferredSessions: 1, releasedClaims: 0, prunedTelegramUpdates: 3, deletedStoredObjects: 2, failedStoredObjectDeletes: 1 });
