...
describe('handoff dispatch workflow', () => {
  ...
  it('keeps the dispatch time budget below its HTTP and workflow limits', async () => {
    ...
    expect(HANDOFF_DISPATCH_BATCH_SIZE).toBe(2);
    ...
    expect(HANDOFF_DISPATCH_BATCH_SIZE * (HANDOFF_SEND_TIMEOUT_MS + TOPIC_CREATE_TIMEOUT_MS + cleanupTimeoutMs)).toBeLessThanOrEqual(dispatchHttpTimeoutMs - 20_000);
