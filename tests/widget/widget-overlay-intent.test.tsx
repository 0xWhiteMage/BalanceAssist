...
describe('intake short replies stay on the LLM path', () => {
  test('sending "who are you" during the service step still routes through /api/chat', async () => {
    ...
    global.fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes('/api/chat') && init?.method === 'POST') {
        ...
        return new Response(
          JSON.stringify({
            message: 'I can answer that later, but first tell me the kind of support you need for this project.',
            draftUpdates: {},
            briefReady: false,
            reviewPrompt: null,
            missingFields: ['service', 'timelineBand', 'budgetBand', 'contact']
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/I can answer that later, but first tell me the kind of support you need/i)
      ).toBeInTheDocument();
    }, { timeout: 4000 });

    expect(screen.queryByText(/I'm \*\*Balance Assist\*\*/i)).toBeNull();
